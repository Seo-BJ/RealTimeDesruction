...
if (Vertex.IsValid())
{
	...
	PriorityQueue.Insert(Bisector);
		...
		{
			TSharedPtr<FVoronoiDiagramEdge, ESPMode::ThreadSafe> e = (*Itr);
			e->GenerateClippedEndPoints(Bounds);
		}
		...
		for (auto SiteItr(Sites.CreateConstIterator()); SiteItr; ++SiteItr)
		{
			...
			{
				TSharedPtr<FVoronoiDiagramEdge, ESPMode::ThreadSafe> Edge_ = (*EdgeItr);
				
				// Only add edges that are visible
				// Don't need to check the Right because they will both be FLT_MIN
				if (Edge_->LeftClippedEndPoint == FVector2D(FLT_MIN, FLT_MIN))
				{
					continue;
				}

				GeneratedSite.Edges.Add(FVoronoiDiagramGeneratedEdge(Edge_->Index, Edge_->LeftClippedEndPoint, Edge_->RightClippedEndPoint));

				if (Edge_->LeftSite.IsValid())
				{
					GeneratedSite.NeighborSites.AddUnique(Edge_->LeftSite->Index);
				}

				if (Edge_->RightSite.IsValid())
				{
					GeneratedSite.NeighborSites.AddUnique(Edge_->RightSite->Index);
				}
			}
		...
		{
			FVoronoiDiagramGeneratedSite CurrentSite_ = Itr->Value;

			FIntPoint CentroidPoint(FMath::RoundToInt(CurrentSite_.Centroid.X), FMath::RoundToInt(CurrentSite_.Centroid.Y));
...
void FVoronoiDiagramHelper::GenerateTexture(TSharedPtr<FVoronoiDiagram> VoronoiDiagram, UTexture2D*& GeneratedTexture)
{
    ...
    FColor* MipData = static_cast<FColor*>(GeneratedTexture->GetPlatformData()->Mips[0].BulkData.Lock(LOCK_READ_WRITE));
    for(int32 x = 0; x < VoronoiDiagram->Bounds.Width(); ++x)
    {
        for(int32 y = 0; y < VoronoiDiagram->Bounds.Height(); ++y)
        {
